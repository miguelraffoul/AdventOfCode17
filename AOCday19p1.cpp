...
string traverseMaze( vector<string>&, const int&, const int& );
...
string traverseMaze( vector<string>& grid, const int& x, const int& y ) {
    string letters;
    ...
    while( true ) {
        ...
        if( grid[y_pos][x_pos] >= 'A' && grid[y_pos][x_pos] <= 'Z' )
            letters += grid[y_pos][x_pos];
